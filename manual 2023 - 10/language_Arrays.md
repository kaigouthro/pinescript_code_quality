...
Where `<type>` is a [type template](language/Type_system.html#pagetypesystem-typetemplates) for the array that declares the type of values it will contain, and the `<expression>` returns either an array of the specified type or `na`.

When declaring a variable as an array, we can use the [array](https://www.tradingview.com/pine-script-reference/v5/#op_array) keyword followed by a [type template](language/Type_system.html#pagetypesystem-typetemplates). Alternatively, we can use the `type` name followed by the `[]` modifier (not to be confused with the [\[\]](https://www.tradingview.com/pine-script-reference/v5/#op_[]) _history-referencing operator_).
